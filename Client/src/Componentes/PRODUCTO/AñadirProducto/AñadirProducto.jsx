...
const handleImageChange = (event) => {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log(input);
    try {
      const response = await axios.post (`http://localhost:3001/crearProducto`, input)
    } catch (error) {
      console.log(error);
    }
  }
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      <div>
        ...
        <label className={styles.inputGropLabel} htmlFor="name">Nombre: </label>
        <input className={styles.inputGroup} type="text" id="name" name='nombre' value={input.nombre} onChange={handleChange} /> 
      </div>
      <div>
        <label className={styles.inputGropLabel} htmlFor="price">Precio: </label>
        <input className={styles.inputGroup}type="text" id="price" name='precio' value={input.precio} onChange={handleChange} /> 
      </div>
      <div>
        <label className={styles.inputGropLabel} htmlFor="image">Imagen: </label>
        <input className={styles.inputGroup}type="file" id="image" name='img' value={input.img} onChange={handleImageChange} />
        <br />
        <br />
        {/* {input.img && (
      ...
      <button className={styles.buttonDelete} type="button" onClick={handleRemoveImage}>Eliminar imagen</button>
      <br />
      <button className={styles.buttonCreate} type="submit" onClick={handleSubmit}>Crear Producto</button>
