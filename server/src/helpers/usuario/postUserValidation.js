...
const postUserValidation = (nombre, telefono, email) => {
    //valido el nombre
    if(!nombreRegex.test(nombre)) throw new Error(`El nombre ${nombre} no es válido para crear el usuario.`)
    if(nombre === '' || nombre.length === 0) throw new Error(`Es necesario que el campo usuario contenga un nombre para poder crearlo.`);
