...
const updateProductoHandler = async (req, res, next) => {
  const { id } = req.params;
  const { nombre,img,precio,estado,cantidad,stock,ventaId } = req.body;

  try {
    const respuesta = (typeof estado!=="boolean")? await updateProducto(id,nombre,img,precio,cantidad,stock,ventaId): await deleteProducto(id,estado);
